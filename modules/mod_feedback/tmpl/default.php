 ...
 * Some Changes for joomla 2.5 done by -Atul Kainth - atul.kainth@gmail.com
 */
?>
...
<?php
	...
	$sliderButton = '<div id="feedback-slider">&nbsp;</div>';
?>
<?php 
/*
...
*/
$start_state =  $params->get('start_state','close');
...
<style type="text/css">
	#feedback-div {
		...
		top: <?php echo $params->get('mod_top','180'); ?>px;
	...
	#feedback-div dl { margin:10px 0px; }
	...
	#feedback-div .ajax-loading {
		background: url('<?php echo JURI::base(); ?>modules/mod_feedback/images/ajax-loader.gif') right top no-repeat;
	}
...
<div id="feedback-div" class="<?php echo $params->get('moduleclass_sfx'); ?>">
	<?php //if($params->get('location') == 'right')
	echo $sliderButton; ?>
	<div id="feedback-info">
		...
		<form id="feedback-form" action="<?php echo JRoute::_('index.php'); ?>" method="post">
			...
			<input type="submit" id="submit-feedback" class="feedback-button" value="<?php echo JText::_('Send feedback'); ?>" />
...
<script type="text/javascript">
	...
	window.addEvent(((window.webkit) ? 'load':'domready'), function() {
		...
		feedbacktoggle = new Fx.Tween('feedback-div',{property :'<?php echo $params->get('location','right'); ?>',  duration: 1000 });
		...
		$('feedback_msg').addEvent('click', function(e) {
			this.set('html','');
			this.setOpacity(0);
		});
		...
		$('feedback-form').addEvent('submit', function(e) {			
			...
			var flooded = false;
			<?php $flood_limit = $params->get('flood_limit','5'); ?>
			...
			if(errMsg == '') {
				 ...
				 */
				var req = new Request({
						url: '<?php echo JRoute::_('index.php'); ?>/%5Bobject%20Object%5D',
						...
						onComplete: function(response) {
							...
							var messagediv = new Fx.Tween('feedback_msg',{property:'opacity', duration: 1000});
							messagediv.set(1);
							messagediv.set('visibility','visible');
	...
	$('feedback-slider').addEvent('click', function(ev) {	
		...
		var now = $('feedback-div').getStyle('<?php echo $params->get('location','right'); ?>').toInt();
