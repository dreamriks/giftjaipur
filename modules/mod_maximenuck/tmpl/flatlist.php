...
foreach ($items as $i => &$item) {
    ...
    if (isset($item->content) AND $item->content) {
        echo '<li class="maximenuflatlistck '. $item->classe . ' level' . $item->level .' '.$item->liclass . '">' . $item->content;
		$item->ftitle = '';
    }
    ...
    if ($item->ftitle != "") {
        ...
        if ($params->get('imageonly', '0') == '1')
            $item->ftitle = '';
        echo '<li class="maximenuflatlistck '. $item->classe . ' level' . $item->level .' '.$item->liclass . '" style="z-index : ' . $zindex . ';">';
        switch ($item->type) :
            default:
